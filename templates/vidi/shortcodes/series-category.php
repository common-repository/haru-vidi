...
$shortcode_id = 'series-category-shortcode' . rand();
...
$atts = compact('layout', 'columns', 'categories', 'series_style', 'posts_per_page', 'filter', 'filter_all', 'orderby', 'order', 'view_more');
...
?>
<?php if ( have_posts() ) : ?>
    <div class="series-category-shortcode <?php echo esc_attr( $layout . ' ' . $extra_class); ?>" data-atts="<?php echo htmlentities( json_encode($atts) ); ?>" id="<?php echo esc_attr( $shortcode_id ) ?>">
        <?php
        ...
        ?>
        <div class="series-category-top">
            <?php if ( $title != '' ) : ?>
                <h6 class="series-category-title haru-shortcode-title"><?php echo esc_html( $title ); ?></h6>
            <?php endif; ?>

            <?php if ( $filter == 'show' ) : ?>
            <ul class="series-filter">
            ...
            <div class="series-category-content">
                ...
                <?php if ( in_array($layout, array('default') ) && $view_more == 'button' ) : ?>
                    <div class="seriess-ajax-view-more <?php echo esc_attr( $view_more == 'button' ? '' : 'hide' ); ?>">
                        <a href="<?php echo esc_url( ( $filter_all == 'show' || $filter == 'hide' ) ? get_post_type_archive_link('haru_series') : get_term_link(explode(',', $categories)[0], 'series_category') ); ?>" class="button-background button-background--primary button-background--small"><?php echo esc_html__( 'View more', 'haru-vidi' ); ?></a>
                    </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="series-control <?php echo esc_attr( $view_more == 'arrow' ? '' : 'hide-control' ); ?>">
