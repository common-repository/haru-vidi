...
$shortcode_id = 'video-my-videos-shortcode' . rand();
...
if ( is_user_logged_in() ) :
    ...
    $args = array(
        ...
        'paged'          => $paged,
        'post_status'    => 'publish', // @TODO: can add option pending video when edit
        'author'         => $author_id
    );
    ...
    <?php if ( have_posts() ) : ?>
        <div class="video-my-videos-shortcode <?php echo esc_attr( $layout . ' ' . $extra_class); ?>" id="<?php echo esc_attr( $shortcode_id ) ?>">
            <div class="haru-archive-top">
                <div class="haru-archive-top-left">
                    <h6 class="archive-video__title"><?php echo esc_html__( 'You has total', 'haru-vidi' ); ?>
            ...
            <div class="video-list layout-wrap style-grid grid-columns grid-columns__<?php echo esc_attr( $columns ); ?>">
                <?php while ( have_posts() ) : the_post(); ?>
                    <?php echo haru_vidi_get_shortcode_template('vidi/video/'. 'content-video' . '.php', array( 'video_style' => $video_style, 'can_edit' => true ), '', ''); ?>
                <?php endwhile; ?>
            </div>
    ...
    <?php else : ?>
        <h6 class="haru-notice"><?php echo esc_html__( 'You don\'t created any videos yet!', 'haru-vidi' ) ?></h6>
    <?php endif;
    wp_reset_query();
...
<?php else : ?>
    <h6 class="haru-notice"><?php echo esc_html__( 'Please login to see your videos!', 'haru-vidi' ); ?></h6>
<?php endif; ?>