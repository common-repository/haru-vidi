...
$shortcode_id = 'video-top-shortcode' . rand();
...
$atts = compact('layout', 'title', 'categories', 'order_by', 'order', 'video_style', 'posts_per_page'); 
...
?>
<?php if ( have_posts() ) : ?>
    <div class="video-top-shortcode <?php echo esc_attr( ($dark_style == 'yes') ? 'dark-style' : '' ); ?> <?php echo esc_attr( $layout . ' ' . $extra_class); ?>" 
        id="<?php echo esc_attr( $shortcode_id ); ?>" 
        data-atts="<?php echo htmlentities( json_encode($atts) ); ?>"
        data-video_order_by="<?php echo esc_attr( $order_by ); ?>"
    >
        <?php if ( $title != '' ) : ?>
            <h6 class="video-top-title haru-shortcode-title"><span><?php echo esc_html( $title ); ?></span></h6>
        <?php endif; ?>
        ...
        <div class="video-control <?php echo esc_attr( ($wp_query->max_num_pages > 1) ? '' : 'hide' ); ?>">
