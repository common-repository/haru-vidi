...
?>
<?php if ( have_posts() ) : ?>
    <div class="channel-slideshow-shortcode <?php echo esc_attr( $layout . ' ' . $extra_class); ?>">
        <?php if ( $title != '' ) : ?>
            <h6 class="channel-slideshow-title haru-shortcode-title"><span><?php echo esc_html( $title ); ?></span></h6>
        <?php endif; ?>

        <?php if ( in_array($layout, array('default') ) ) : ?>
        <div class="channel-list haru-slick" 
        ...
        >
            <?php while ( have_posts() ) : the_post(); ?>
                <?php echo haru_vidi_get_shortcode_template('vidi/channel/'. 'content-channel' . '.php', array( 'channel_style' => $channel_style ), '', ''); ?>
            <?php endwhile; ?>
        </div>
        <?php endif; ?>
    </div>
