...
$archive_layout_full = haru_vidi_get_setting( 'vidi-videos-settings', 'archive_videos_settings_layout_full', 'off' );
$archive_layout_sidebar = haru_vidi_get_setting( 'vidi-videos-settings', 'archive_videos_settings_sidebar', 'sidebar-none' );
$archive_sidebar_left = haru_vidi_get_setting( 'vidi-videos-settings', 'archive_videos_settings_sidebar_left', '' );
$archive_sidebar_right = haru_vidi_get_setting( 'vidi-videos-settings', 'archive_videos_settings_sidebar_right', '' );
$archive_columns = haru_vidi_get_setting( 'vidi-videos-settings', 'archive_videos_settings_columns', '2' );
$archive_per_page = haru_vidi_get_setting( 'vidi-videos-settings', 'archive_videos_settings_per_page', '' );
$archive_paging_style = haru_vidi_get_setting( 'vidi-videos-settings', 'archive_videos_settings_paging_style', 'default' );
...
<div class="haru-archive-video <?php echo esc_attr( $archive_layout_full == 'on' ? '' : 'haru-container' ); ?>">
    <!-- Content -->
    <div class="archive-content <?php if ( is_active_sidebar( $archive_sidebar_left ) && in_array($archive_layout_sidebar, array( 'sidebar-left', 'two-sidebar' ) ) ) echo esc_attr( 'has-left-sidebar' ); ?> <?php if ( is_active_sidebar( $archive_sidebar_right ) && in_array($archive_layout_sidebar, array( 'sidebar-right', 'two-sidebar' ) ) ) echo esc_attr( 'has-right-sidebar' ); ?>">
        ...
        <div class="haru-archive-top">
            <div class="haru-archive-top-left">
                <h6 class="archive-video__title"><?php echo esc_html__( 'Has total', 'haru-vidi' ); ?>
                    <span class="archive-video__total-count"><?php echo esc_html( $count ) . esc_html__( ' videos', 'haru-vidi' ); ?></span>
                </h6>
            </div>
            <div class="haru-archive-top-right">
                ...
                <div class="haru-archive-order">
                    <div class="order-item-current">
                        <?php
                            if ( $orderby == 'date' ) {
                                echo esc_html__( 'Date', 'haru-vidi' );
                            } elseif ( $orderby == 'title' ) {
                                echo esc_html__( 'Title', 'haru-vidi' );
                            }
                        ?>
                    </div>
        ...
        <?php if ( have_posts() ) : ?>
        <div class="archive-video-list layout-wrap style-grid grid-columns grid-columns__<?php echo esc_attr( $archive_columns ); ?>">
            <?php
                // Start the Loop.
