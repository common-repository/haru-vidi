...
class Haru_Playlist_Top_Widget extends Haru_Vidi_Widget {
    ...
    public function __construct() {
        $this->widget_cssclass    = 'widget-playlist-top';
        $this->widget_description = esc_html__( 'Widget display top playlists.', 'haru-vidi' );
        $this->widget_id          = 'haru_widget_playlist_top';
        $this->widget_name        = esc_html__( 'Haru TOP Playlists', 'haru-vidi' );
        $this->cached             = false;
        $this->settings = array(
            'title'         => array(
                'type'  => 'text',
                'std'   => esc_html__( 'Top Playlists', 'haru-vidi' ),
            ...
            'number'      => array(
                ...
                'max'   => '',
                'std'   => 5,
                'label' => esc_html__( 'Number of playlists to show', 'haru-vidi' ),
            ),
            'orderby'     => array(
                'type'    => 'select',
                'std'     => 'like',
    ...
    public function widget( $args, $instance ) {
        ...
        if ( $playlists && $playlists->have_posts() ) :
            echo $before_widget;
                if ( $title ) echo $before_title . $title . $after_title;
                echo '<ul class="playlists-list ' . esc_attr( $style ) . '">';
                while ( $playlists->have_posts() ) : $playlists->the_post();
                    echo haru_vidi_get_shortcode_template('vidi/widgets/'. 'widget-playlist-top' . '.php', $instance, '', '');
                endwhile;
                echo  '</ul>';
            echo $after_widget;
        endif;
...
if ( !function_exists( 'haru_register_widget_playlist_top' ) ) {
    function haru_register_widget_playlist_top() {
        register_widget( 'Haru_Playlist_Top_Widget' );
    }
    add_action( 'widgets_init', 'haru_register_widget_playlist_top' );
}