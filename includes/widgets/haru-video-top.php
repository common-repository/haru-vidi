...
class Haru_Video_Top_Widget extends Haru_Vidi_Widget {
    ...
    public function __construct() {
        $this->widget_cssclass    = 'widget-video-top';
        $this->widget_description = esc_html__( 'Widget display top videos.', 'haru-vidi' );
        $this->widget_id          = 'haru_widget_video_top';
        $this->widget_name        = esc_html__( 'Haru TOP Videos', 'haru-vidi' );
        $this->cached             = false;
        $this->settings = array(
            'title'         => array(
                'type'  => 'text',
                'std'   => esc_html__( 'Top Videos', 'haru-vidi' ),
        ...
        if ( $videos && $videos->have_posts() ) :
            echo $before_widget;
                ...
                while ( $videos->have_posts() ) : $videos->the_post();
                    echo haru_vidi_get_shortcode_template('vidi/widgets/'. 'widget-video-top' . '.php', $instance, '', '');
                endwhile;
                echo  '</ul>';
            echo $after_widget;
        endif;
...
if ( !function_exists( 'haru_register_widget_video_top' ) ) {
    function haru_register_widget_video_top() {
        register_widget( 'Haru_Video_Top_Widget' );
    }
    add_action( 'widgets_init', 'haru_register_widget_video_top' );
}