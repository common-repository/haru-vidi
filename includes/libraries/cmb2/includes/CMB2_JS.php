...
class CMB2_JS {
	...
	protected static function colorpicker_frontend() {
		...
		wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
			'clear'         => esc_html__( 'Clear', 'haru-vidi' ),
			'defaultString' => esc_html__( 'Default', 'haru-vidi' ),
			'pick'          => esc_html__( 'Select Color', 'haru-vidi' ),
			'current'       => esc_html__( 'Current Color', 'haru-vidi' ),
		) );
	}
		...
		$l10n = array(
			...
			'defaults'          => array(
				...
				'date_picker'  => array(
					'changeMonth'     => true,
					'changeYear'      => true,
					'dateFormat'      => _x( 'mm/dd/yy', 'Valid formatDate string for jquery-ui datepicker', 'haru-vidi' ),
					'dayNames'        => explode( ',', esc_html__( 'Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday', 'haru-vidi' ) ),
					'dayNamesMin'     => explode( ',', esc_html__( 'Su, Mo, Tu, We, Th, Fr, Sa', 'haru-vidi' ) ),
					'dayNamesShort'   => explode( ',', esc_html__( 'Sun, Mon, Tue, Wed, Thu, Fri, Sat', 'haru-vidi' ) ),
					'monthNames'      => explode( ',', esc_html__( 'January, February, March, April, May, June, July, August, September, October, November, December', 'haru-vidi' ) ),
					'monthNamesShort' => explode( ',', esc_html__( 'Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec', 'haru-vidi' ) ),
					'nextText'        => esc_html__( 'Next', 'haru-vidi' ),
					'prevText'        => esc_html__( 'Prev', 'haru-vidi' ),
					'currentText'     => esc_html__( 'Today', 'haru-vidi' ),
					'closeText'       => esc_html__( 'Done', 'haru-vidi' ),
					'clearText'       => esc_html__( 'Clear', 'haru-vidi' ),
				),
				'time_picker'  => array(
					'timeOnlyTitle' => esc_html__( 'Choose Time', 'haru-vidi' ),
					'timeText'      => esc_html__( 'Time', 'haru-vidi' ),
					'hourText'      => esc_html__( 'Hour', 'haru-vidi' ),
					'minuteText'    => esc_html__( 'Minute', 'haru-vidi' ),
					'secondText'    => esc_html__( 'Second', 'haru-vidi' ),
					'currentText'   => esc_html__( 'Now', 'haru-vidi' ),
					'closeText'     => esc_html__( 'Done', 'haru-vidi' ),
					'timeFormat'    => _x( 'hh:mm TT', 'Valid formatting string, as per http://trentrichardson.com/examples/timepicker/', 'haru-vidi' ),
					'controlType'   => 'select',
					'stepMinute'    => 5,
				),
			),
			'strings' => array(
				'upload_file'  => esc_html__( 'Use this file', 'haru-vidi' ),
				'upload_files' => esc_html__( 'Use these files', 'haru-vidi' ),
				'remove_image' => esc_html__( 'Remove Image', 'haru-vidi' ),
				'remove_file'  => esc_html__( 'Remove', 'haru-vidi' ),
				'file'         => esc_html__( 'File:', 'haru-vidi' ),
				'download'     => esc_html__( 'Download', 'haru-vidi' ),
				'check_toggle' => esc_html__( 'Select / Deselect All', 'haru-vidi' ),
			),
		);
